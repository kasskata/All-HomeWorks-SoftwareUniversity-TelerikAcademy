var ajaxRequester = (function () {
    ...
    function register(acc, pass, success, error) {
        var data = JSON.stringify({username: acc, password: pass});
        return makeRequest('POST', baseUrl + "users", data, headers, success, error);
    }

    function makeGetRequest(sessionToken, success, error) {
        return makeRequest('GET', baseUrl + 'classes/Bookmark', null, includeToken(sessionToken), success, error);
    }

    function makePostRequest(sessionToken, data, success, error) {
        return makeRequest('POST', baseUrl + 'classes/Bookmark', data, includeToken(sessionToken), success, error);
    }

    function makePutRequest(sessionToken, url, data, success, error) {
        return makeRequest('PUT', url, data, includeToken(sessionToken), success, error);
    }

    function makeDeleteRequest(id, sessionToken, success, error) {
        return makeRequest('DELETE', baseUrl + 'classes/Bookmark/' + id, null, includeToken(sessionToken), success, error);
    }
